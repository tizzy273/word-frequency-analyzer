...
The Word Frequency Analyzer is a microservice for analyzing word frequencies in text documents. It provides endpoints for calculating the highest frequency, the frequency of a specific word, and the most frequent N words. Words are sequences of letters (a-z), case-insensitive, and non-letter characters act as separators. Once the application is running, API documentation is available through [Swagger](http://localhost:8080/swagger-ui.html).

To run locally, clone the repository to your machine and make sure you have **JDK 25** installed, then run the project:
